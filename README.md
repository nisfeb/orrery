...
Messages and calendar actions follow the claim protocol: claimed `by` `ship`, then `done` with `sent to <person> by telegram`, `sent to <person> by mail`, `sent to <person> on Urbit`, `on the calendar`, `off the calendar` or `that occurrence skipped`, or `failed` with the reason (Telegram's own description, auspex's refusal, or, for a cancel, `the calendar does not have that event` or `that event repeats, so the occurrence is needed`).
...
A proposed action on the page carries a box under it for a note (version 36): "include dana in this", "make it 3pm", "send this as mail". `POST /actions/<id>/refine` runs one model call and rewrites the action's title, payload, `about` and `due` in place, keeping its id and history and appending one step `revised` by whoever asked. A refine is a model call like the generator's: it counts against the generator's `max_daily` and its cost is added to the month's.
...
### Text a client hands in

Since version 59 a client can hand the ship text to read: `POST /read` with the text, a title, the source (a web page's url) and who is speaking, and the read fiber runs it through the reader's pipeline exactly as a message, so a page you send from the browser extension becomes facts with the page as their source and proposals to approve. The owner is the speaker unless the client says otherwise, since the owner chose to hand the text in. Text a key hands in is read as that key: its facts are signed by the key's `by`, and only what the key could have filed itself is kept, the rest dropped with a note in the record. The channel's `max_daily_messages` holds what is over the day's count until tomorrow. The Read card under Settings holds the channel's settings and its last read; a text the model could not read waits and is tried again in five minutes.

### Mail, and the daily brief

Since version 52 the ship reads the mail auspex holds, the way a phone client's mail reader did: every few minutes (the Mail card, `GET`/`PUT /mail`, on by default when auspex is installed) it walks auspex's mail tree, and every message from someone else sent in the last week (the first pass reads back `backfill_hours`) and not yet read goes through the reader's pipeline, one run per thread, the subject heading the text; a thread archived in auspex, a copy auspex has not verified as its sender's and a blank body are left alone, a sender in your Tlon contact book with no person body is made one (`person/<ship>`, named by its ship) while a ship you never added stays a stranger, and each message read gives its sender a `last-contact` for that day. A mail auspex refuses (its trail says so) fails the action with auspex's reason. The record is `GET /mail/last`, the wake `POST /mail/wake`.
...
An approved `message` whose `via` is `chat` is sent by the ship as a Tlon DM to the person's `ship`, or, when its payload names a `channel` (a nest such as `chat/~host/general`), posted in that channel, through the kernel's gall road, when `send_dms` is on in the chat reader's settings (the Chat card). It needs the kernel to carry the typed marc for `chat-dm-action-2` (grubbery `gub/mar/clay/groups/chat/dm/action-2.hoon`): a kernel without it crashes on the poke and the executor stands at it until the instance reloads, so with the switch off the message is left approved for the client that sends chat, noted in the executor's record. The switch is on by default.
...
A part of orrery that crashes, or that loses a road it needs because a permit is refused or unticked, never takes the ship down with it (version 60). Each long-running part (the writer, the readers, the brief, the executor, the generator, reconcile, sync) waits and then goes on by itself: a minute after the first crash, then 2, 4 and up to 60 minutes if it crashes again, the count starting over after two quiet hours. Only the first two crashes print their whole trace. While a part waits, anything sent to it is refused at once, so a write to a crashed writer fails where you can see it instead of vanishing. With the clock or the timer refused it waits for a poke instead, and approving the permits (which reloads the app) brings it back. `rise.json` in the instance holds each part's crashes in a row and when it next tries. A request that crashes ends at once.
...
An observation's id is the unix seconds of `at`, a hyphen, and eight hex digits of a hash over subject, attribute, value, time and source, so the same claim from the same source is one row however often it is sent. Retraction keeps the row, and the ship records it as seen when it was retracted. On each write to a body, its superseded, expired and retracted rows older than `retention_days`, by the later of when they were true and when the ship recorded them, are culled; a live row never is, nor the row each attribute would fall back to if its winner were retracted or expired.
...
| `GET /read/last` | the last text read: the reader's record shape, `notes` naming each text; owner only |
| `POST /read/wake` | read what waits now; owner only |
...
- `code/` is the desk: the nexus at `code/nex/orrery/app.hoon` with the page beside it, the model in `code/lib/orrery.hoon` (pure, import-free, unit-tested; since version 60 it also holds the nexus's rules, such as the route table and who may take each route, the request and key checks, the readers' verdicts and the writer's choices, while the nexus keeps the reads and writes and a one-line alias per moved arm), the MCP tools under `code/lib/tools`, the marcs under `code/mar`. `code/version.json` is what replicates. The telegram reader's fibers live in the nexus beside the webhook route; its own settings, window and connections are kept in the instance's `telegram.json`, `telegram-recent.json`, `telegram-last.json` and `telegram-connections.json`, none of them in git. The executor is the fiber `exec.sig` in the nexus, its planners `plan-exec` and `plan-mirror` in the lib, and its record the instance's `exec-last.json`; `code/mar/auspex-action.hoon` is a copy of auspex's marc, since a guest distributes every marc it names. The lib's `system-prompt`, `analyst-prompt` and `refine-prompt` cords are `orrery-utils/common/generator-prompt.md`, `analyst-prompt.md` and `refine-prompt.md`, word for word, held to them by `scripts/prompt-drift.py`.
- `tests/lib/orrery.hoon`, `tests/lib/generator.hoon` and `tests/lib/nexus.hoon` are the unit suites, the last for the nexus's rules, run on a test desk by the vendored [hoon-test-kit](https://github.com/nisfeb/hoon-test-kit) in `scripts/hoon-test-kit/`, as `hoon-test.conf` says; `docs/hoon-testing.md` has the setup and what the mutation runs found.
...
| `scripts/hoon-test-kit/hoon-test.sh <pier>` on a fake ship with the `%orrery-test` desk (`docs/hoon-testing.md`) | the unit suites, in about 17 s with no dojo |
| `scripts/hoon-test-kit/hoon-mutate.py <pier> --since <last release>` | every break a mutant makes in the arms changed since then is caught, or triaged in `docs/hoon-testing.md` |
...
Each prints `ALL OK` or names the first check that failed.

A release that changes start-up, crash handling or anything read from stored state is also tested as an upgrade and against a refusing weir, on a fake ship with the stock grubbery kernel (the rules are in the shared note on grubbery crash loops). For the upgrade, put the previous release's code on the ship and feed it odd data through its own routes with `scripts/upgrade-seed.py $SHIP $JAR`. Then write the new code over it and run `scripts/ship-watch.py <worker pid> $SHIP $JAR 5`: for five minutes the worker's CPU stays idle between probes, the instance's bang stays null and the version route answers. For the weir, take `/sys/behn/`, then `/sys/bowl.sig`, out of the instance's poke weir (the explorer's `del-weir-road`, then `reload-nexus`) and watch the same way: orrery must park, not spin. Put each road back and reload after. A by-hand pass on `/apps/orrery` with the owner cookie closes a release: the views render, a retract and a move take effect, a settings save round-trips, and a write from a second client refreshes the page through the beacon. The release itself is a bump of `code/version.json`; `docs/releasing.md` says why that number is the whole mechanism and how to tell a release that landed from one that did not.
