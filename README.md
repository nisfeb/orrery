...
- `tests/lib/orrery.hoon` and `tests/lib/generator.hoon` are the unit suites for the model, run on a test desk by the vendored [hoon-test-kit](https://github.com/nisfeb/hoon-test-kit) in `scripts/hoon-test-kit/`, as `hoon-test.conf` says; `docs/hoon-testing.md` has the setup and what the mutation runs found.
