...
print('== preferences')
code, before = owner('GET', '/schema')
before = dictish(before)
code, d = writer('PUT', '/preferences', {'preferences': ['key gate preference']})
check('a key with write may set the preferences, and hears what they will be', code == 200 and dictish(d).get('preferences') == ['key gate preference'], (code, d))
time.sleep(1)
code, d = reader('GET', '/preferences')
check('any key reads the style and preferences', code == 200 and dictish(d).get('preferences') == ['key gate preference'], (code, d))
code, d = reader('PUT', '/preferences', {'style': 'x'})
check('a read-only key may not set them', code == 403, (code, d))
code, d = writer('PUT', '/preferences', {'preferences': [5]})
check('a preference that is not a string is 400, the field named', code == 400 and 'preferences' in str(dictish(d).get('error')), (code, d))
code, after = owner('GET', '/schema')
check('the rest of the schema is untouched', {k: v for k, v in dictish(after).items() if k not in ('style', 'preferences')} == {k: v for k, v in before.items() if k not in ('style', 'preferences')}, None)
code, d = writer('GET', '/schema')
