...
- A scope is `{"kinds": [...], "actions": [...], "write": true|false, "sensitive": "none"|"write"}`. `kinds` are the body kinds the client may read and, with `write`, observe; `actions` the action kinds it may propose; `write` also covers approving, dismissing and completing; `sensitive` is `"write"` for a key that may observe the attributes `policy.sensitive` names, which every view goes on hiding from it, and it needs `write`.
